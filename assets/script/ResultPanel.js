...
cc.Class({
  ...
  goNext: function () {
    ...
    if (facade.getComponent('GameModel').energy < n) {
      return cc.systemEvent.emit(i.ENERGY_NEEDED), void this.node.runAction(cc.sequence(cc.delayTime(0.5), cc.callFunc(function () {
  ...
  gotLinks: function (e) {
    if (this.linkBox.removeAllChildren(), !e || e.length == 0) return this.linkBox.opacity = 0, void (this.linkBox.active = !1)
    if (this.linkBox.opacity = 255, this.linkBox.active = !0, window.facade.getComponent('PlayerModel').isClickCountLimit(3)) { for (let t = 0; t < e.length; t++) e[t].navout_max_cnt && e[t].navout_max_cnt != '' && Number(e[t].navout_max_cnt) != 0 && e[t].navout_cnt >= Number(e[t].navout_max_cnt) && (e.splice(t, 1), t--) }
  ...
  checkGfit: function () {
    if (!facade.SAVE_MODE) {
      ...
      e != 0 && window.popUp.getComponent('Pop').addPopByName(e + 'View', null, !0, !0)
    }
  },
  update: function (e) {}
})
