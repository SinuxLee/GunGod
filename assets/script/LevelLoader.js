...
cc.Class({
  ...
  loadLevel: function (e) {
    let t = e.split('_')[0]
    if (t == 'level' && (t = 'classic'), facade.selectedLevelType = t, !this.inLoading && !this.node.getChildByName('levelPlay')) {
    ...
    if (window.facade.getComponent('LevelModel').playingLevelConfig) {
      for (let e = this.level.getChildren(), t = cc.loader.getRes('roles', cc.SpriteAtlas), n = 0; n < e.length; n++) {
        if (e[n].name.match('suffer') != null) {
          for (let i = window.facade.getComponent('LevelModel').playingLevelConfig.Enemy, o = e[n].getChildren(), a = 0; a < o.length; a++) { if (o[a].name != 'blood') o[a].getComponent(cc.Sprite).sizeMode = cc.Sprite.SizeMode.TRIMMED, o[a].getComponent(cc.Sprite).spriteFrame = t.getSpriteFrame(i + '_' + o[a].name)
          else {
  ...
  changeShotSkins: function () {
    if (window.facade.getComponent('LevelModel').nowLevelConfig) {
      for (let e = this.level.getChildren(), t = cc.loader.getRes('roles', cc.SpriteAtlas), n = 0; n < e.length; n++) {
        if (e[n].name.match('suffer') != null) {
          for (let i = window.facade.getComponent('LevelModel').nowLevelConfig.Enemy, o = e[n].getChildren(), a = 0; a < o.length; a++) { if (o[a].name != 'blood') o[a].getComponent(cc.Sprite).sizeMode = cc.Sprite.SizeMode.TRIMMED, o[a].getComponent(cc.Sprite).spriteFrame = t.getSpriteFrame(i + '_' + o[a].name)
          else {
            var s = cc.Color.BLACK
            o[a].getComponent(cc.ParticleSystem).startColor = s.fromHEX('#94F8F0'), o[a].getComponent(cc.ParticleSystem).endColor = s.fromHEX('#94F8F0')
          } }
  ...
  changeRoleSkin2: function (e) {
    for (let t = window.facade.getComponent('GameModel').characterId, n = cc.loader.getRes('roles', cc.SpriteAtlas), i = e.getChildren(), o = 0; o < i.length; o++) {
      const a = i[o]
