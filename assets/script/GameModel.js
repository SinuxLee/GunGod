...
cc.Class({
  ...
  initConfig: function () {
    cc.loader.loadRes('config/SignConfig', this.setConfig.bind(this))
    cc.loader.loadRes('config/CharacterConfig', cc.JsonAsset, this.characterLoaded.bind(this))
  ...
  dealLoadData: function (e) {
    ...
    this.gotGameRecord = true
    this.localCash = t || {}
    console.log('GameModel data:', e)
    facade.getComponent('LevelModel').httpResDeal(this.localCash)
    this.formDatas()
  },
  ...
  formDatas: function (e) {
    this.firstLoginTime1 = this.localCash.firstLoginTime1
    ...
    this.characterId = Number(this.localCash.characterId)
    this.unlockCharacters = this.localCash.unlockCharacters
    ...
    this.lastFetchColectTime = Number(this.localCash.lastFetchColectTime)
    this.lastFetchSignInTime = Number(this.localCash.lastFetchSignInTime)
    this.fetchDays = Number(this.localCash.fetchDays)
    this.aliveFetchList = this.localCash.aliveFetchList
    this.newbieFetchList = this.localCash.newbieFetchList
    this.lastFetchFloatReward = this.localCash.lastFetchFloatReward
    this.lastFetchClientTime = this.localCash.lastFetchClientTime
    ...
    if (this.lastRewardEnergyTime) {
      ...
      console.log('lastRewardDate:', a.toString())
      console.log('todayDate:', o.toString())
      n < t && (this.rewardEnergyCount = 0, this.lastRewardEnergyTime = this.getLocalTime(), this.levelShotUpdate = 0, this.chanllenTimes = 0, this.freeLuckyVideoTimes = 0, this.cupNumData = 0, this.oneShotLevelCupNum = 0, this.oneShotLevel = 0, this.levelFreeRetry = 0)
    } else this.lastRewardEnergyTime = 0
    this.getLocalTimeHour() >= 21 && this.levelShotUpdate == 0 && (this.levelShotUpdate = 1), this.levelRetryTouch = false
    const s = new Date()
    if (this.energy < this.maxEnergy && this.energyLostTime) {
      const c = s.getTime() - this.energyLostTime
      let r = Math.floor(c / (2 * this.energyCD))
      r + this.energy > 15 && (r = 15 - this.energy)
      this.energy += r
      this.energyLostTime += r * this.energyCD * 2
      this.energy >= this.maxEnergy && (this.energyLostTime = 0)
      this.offlineEnergy(r)
    }
    this.lastFetchColectTime || (this.lastFetchColectTime = 0), this.lastFetchSignInTime || (this.lastFetchSignInTime = 0), this.fetchDays || (this.fetchDays = 0), this.aliveFetchList || (this.aliveFetchList = []), this.newbieFetchList || (this.newbieFetchList = []), this.lastFetchFloatReward || (this.lastFetchFloatReward = 0), this.lastFetchClientTime || (this.lastFetchClientTime = 0), e || (facade.CurrentScene != 'Game' ? facade.needShowSign = true : (facade.needShowSign = false, this.saveData(), cc.systemEvent.emit(ModuleEventEnum.INIT_ROLEINFO_COMPLETED)))
  },

  setShotUpdateData: function (e) {
    this.levelShotUpdate = e
    this.saveData()
  },
  ...
  setOneShotLeves: function () {
    if (facade.getComponent('LevelModel').oneShotConfigList) {
      ...
      for (let e = facade.getComponent('LevelModel').oneShotConfigList.length; this.oneShotLevelList.length < 20;) {
        for (var t = Math.floor(Math.random() * e + 1), n = false, i = 0; i < this.oneShotLevelList.length; i++) {
  ...
  onLevelShotPassed: function () {
    this.showLevelOver = false
    this.oneShotLevel < 20 ? this.oneShotLevel++ : this.oneShotLevel == 20 && (this.showLevelOver = true)
    ...
    window.net.getComponent('Net').httpRequest(window.net.SaveScore, t)
    this.node.runAction(cc.sequence(cc.delayTime(3), cc.callFunc(this.goToShotNextLevel, this))), this.saveData()
  },

  goToShotNextLevel: function () {
    this.showLevelOver ? cc.systemEvent.emit(ModuleEventEnum.SHOT_NOW_OVER_SHOW) : cc.systemEvent.emit(ModuleEventEnum.GO_SHOT_NEXT)
  },

  onLevelShotNext: function () {
    this.oneShotLevel < 20 ? this.oneShotLevel++ : this.oneShotLevel == 20 && this.setOneShotLeves(), cc.systemEvent.emit(ModuleEventEnum.GO_SHOT_NEXT), this.saveData()
  },
  ...
  offlineEnergy: function (e) {
    console.log('offlineEnergy:', e)
    this._offLineEnergy = e
    this.saveData()
  },

  characterLoaded: function (e, t) {
    console.log('asset:', t)
    this.charaterConfig = t.json
    cc.systemEvent.emit(ModuleEventEnum.CHARACTER_INFO)
  },
  ...
  isCharacterLocked: function (e) {
    for (let t = 0; t < this.unlockCharacters.length; t++) { if (this.unlockCharacters[t] == e) return false }
    return true
  },
  ...
  onLoad: function () {
    ...
    this.oneShotLevelList = []
    this.oneShotLevel = 1
    this.oneShotLevelCupNum = 0
    this.levelShotUpdate = 0
    this.chanllenTimes = 0
    this.cupNumData = 0
    this.levelFreeRetry = 0
    cc.systemEvent.on(ModuleEventEnum.GOT_HTTP_RES, this.httpResDeal.bind(this))
    cc.systemEvent.on(ModuleEventEnum.ROLE_SELECTED, this.onRoleSelected.bind(this))
    cc.systemEvent.on(ModuleEventEnum.LEVEL_SHOT_PASSED, this.onLevelShotPassed, this)
    facade.isMiniGame ? cc.systemEvent.on(ModuleEventEnum.WX_REGISTERED, this.onGotToken.bind(this)) : this.readDataFromLocal()
  },
  ...
  gainEnergy: function () {
    const e = new Date()
    this.energy++
    this.energyLostTime = e.getTime()
    this.energy >= this.maxEnergy && (this.energy = this.maxEnergy, this.energyLostTime = 0), this.saveData()
  },
  ...
  rewardEnergy: function (e) {
    ...
    this.energy += e
    this.energy >= this.maxEnergy && (this.energyLostTime = 0), this.saveData()
  },
  ...
  buyACharacter: function (e) {
    ...
    Number(t.Cost) > this.cash ? popUp.getComponent('FloatTip').showTip('钱钱好像不够哦！') : (cc.systemEvent.emit(ModuleEventEnum.CHARACTER_GOT, e), this.cash -= Number(t.Cost), this.unlockCharacters.push(e), this.characterId = e, this.saveData())
  },
  ...
  addAWatchCount: function () {
    ...
    this.characterVideoCounts[this._watchingId + ''] >= e.Cost && this.unlockCharacters.push(this._watchingId), cc.systemEvent.emit(ModuleEventEnum.SKIN_WATCHED), this._watchingId = null, this.saveData()
  },
  ...
  getCollectReward: function () {
    ...
    this.lastFetchColectTime = t
    this.addCash(parseInt(e.Value))
    const n = '成功领取奖励: 钞票 x ' + e.Value
    popUp.getComponent('FloatTip').showTip(n)
    cc.systemEvent.emit(ModuleEventEnum.UPDATE_COLLECT_SHOW)
  },

  gainSkin: function (e) {
    this.unlockCharacters.push(parseInt(e))
    this.saveData()
  },
  ...
  applyInviteNewbieReward: function (e, t) {
    ...
    this.newbieFetchList.length >= 4 || (this.newbieFetchList.push({
      id: e,
      user_id: t
    }), this.addCash(Number(n.Value)), this.saveData(), cc.systemEvent.emit(ModuleEventEnum.INVITE_UPDATE_DAILY, e))
  },
  applyInviteAliveReward: function (e, t) {
    const n = facade.getComponent('ShareADModel').GameConfig.InviteT
    this.aliveFetchList.length >= 4 || (this.aliveFetchList.push(t), this.rewardEnergy(parseInt(n.Value)), this.saveData(), cc.systemEvent.emit(ModuleEventEnum.INVITE_UPDATE_DAILY, e))
  },
  applyHoverWinReward: function () {
    const e = facade.getComponent('ShareADModel').GameConfig.Float_Reward.Value
    this.lastFetchFloatReward = this.getLocalTime()
    this.rewardEnergy(parseInt(e))
    popUp.getComponent('FloatTip').showTip('恭喜获得体力 x' + e)
    cc.systemEvent.emit(ModuleEventEnum.UPDATE_HOVER_SHOW)
  },
  ...
  applyClientReward: function () {
    ...
    this.rewardEnergy(parseInt(e)), popUp.getComponent('FloatTip').showTip('公众号每日奖励：体力 x' + e), cc.systemEvent.emit(ModuleEventEnum.UPDATE_HOVER_SHOW)
  },
  ...
  waitingForBroadcastReward: function () {
    cc.director.getScene().name == 'Game' && facade.fromBroadcast && (this.checkClientRewardShow() ? (facade.fromBroadcast = false, this.applyClientReward()) : facade.fromBroadcast = false)
  },
  update: function () {
